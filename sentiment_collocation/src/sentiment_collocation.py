...
def collocation(text, filename):
    ...
    filtered_sentence = [w for w in word_tokens if not w.lower() in stopwords_]
    ...
    keyword_freq = text.count(key)
    # corpus size
    corpus_size = len(text)
    
    print(f"There is {corpus_size} words in the text")
...
def main():
    ...
    # wordcloud
    wordcloud(df, args["filename"])
    # collocation 
    collocation(text, args["filename"])
